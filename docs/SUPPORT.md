...
4. To remove local page history, restoration state, preferences, and embedded website data, delete Perch's containers from the current user's `~/Library/Application Support`, `~/Library/Preferences`, and `~/Library/WebKit` folders. Search for `Perch` or `com.fantomsuj.Perch` and verify each exact target before deleting it.
5. In System Settings → General → Login Items & Extensions, remove a stale Perch entry if macOS still shows one.
