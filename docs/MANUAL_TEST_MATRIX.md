...
| Hidden PiP under memory pressure | Trigger memory pressure or wait for warm WebView eviction, then restore and revisit a page | Opaque interaction state may be discarded, but the saved Notion URL and best-effort scroll fallback remain available |  |  |  |
