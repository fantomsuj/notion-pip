...
- **System permissions:** Perch's native 0.1 features do not request Accessibility, screen-recording, microphone, camera, Contacts, or location access. Website features presented by Notion remain subject to Notion's behavior and macOS permission controls. Launch at Login is changed only when the user operates its Settings toggle.

## Quick Copy

Quick Copy is deferred from Perch 0.1. The 0.1 app has no Quick Copy control, does not start its Accessibility selection monitor, and does not request Accessibility permission for that feature.
