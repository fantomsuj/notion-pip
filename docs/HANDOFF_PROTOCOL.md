...
The nested `url` must use HTTPS, have no username or password, use the exact host `notion.so` or `www.notion.so`, and end in a component containing a 32-hex-character Notion page ID. Hyphens in a UUID are ignored for identity. Accepted pages canonicalize to `https://www.notion.so/<original-last-component>` with no query or fragment.
