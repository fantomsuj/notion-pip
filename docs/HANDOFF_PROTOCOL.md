...
The nested `url` must use HTTPS, have no username or password, use the exact host `app.notion.com`, `notion.so`, or `www.notion.so`, and end in a component containing a 32-hex-character Notion page ID. Hyphens in a UUID are ignored for identity. Accepted `notion.so` and `www.notion.so` pages canonicalize to `https://www.notion.so/<original-last-component>`; accepted `app.notion.com` pages retain that host. Canonical URLs have no query or fragment.
