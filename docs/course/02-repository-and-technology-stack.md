...
Perch is one native macOS executable with two authoring toolchains. Swift
...
2. Explain how Swift Package Manager turns `Sources/Perch` and its copied
...
`dist/Perch.app`; signing binds code, bundle contents, and entitlements;
...
| SwiftUI | Declare app views and bind them to observable state | [`Sources/Perch/Views`](../../Sources/Perch/Views) |
| AppKit | Own application lifecycle, panels, menus, shortcuts, and windows | [`Sources/Perch/Platform`](../../Sources/Perch/Platform) |
| WebKit | Host one live Notion session and a separate local editor bridge | [`NotionWebSession.swift`](../../Sources/Perch/Platform/NotionWebSession.swift), [`CaptureEditorSession.swift`](../../Sources/Perch/Platform/CaptureEditorSession.swift) |
| SwiftData | Persist pages, restoration state, captures, and destinations behind actor-backed repositories | [`Sources/Perch/Persistence`](../../Sources/Perch/Persistence) |
| Security/Keychain | Store the optional personal integration token outside preferences and web content | [`PersonalTokenCredentialVault.swift`](../../Sources/Perch/Platform/PersonalTokenCredentialVault.swift) |
| Carbon | Register global keyboard shortcuts | [`GlobalShortcutRegistrar.swift`](../../Sources/Perch/Platform/GlobalShortcutRegistrar.swift) |
| OSLog/signposts | Emit privacy-aware diagnostics and performance intervals | [`PerformanceSignposter.swift`](../../Sources/Perch/Platform/PerformanceSignposter.swift) |
...
├── Tests/PerchTests/      # Swift Testing regression suite and test support
...
package.json + Web/QuickCaptureEditor (only when node_modules exists)
        ...
        Sources/.../Resources/QuickCapture/editor.js
                         │
Package.swift + Sources/Perch + copied QuickCapture resource directory
                         │ swift build --product Perch
                         ▼
...
4. Recreates `dist/Perch.app`, copies the executable into
...
| `./script/build_and_run.sh --logs` | Launch and stream unified logs for the `Perch` process. |
| `./script/build_and_run.sh --telemetry` | Launch and stream logs for subsystem `com.fantomsuj.Perch`. |
...
- executable product `Perch` backed by executable target `Perch` at
  [`Sources/Perch`](../../Sources/Perch);
- test target `PerchTests` at
  [`Tests/PerchTests`](../../Tests/PerchTests), depending on the
  executable target;
...
[`index.html`](../../Sources/Perch/Resources/QuickCapture/index.html),
[`composer.css`](../../Sources/Perch/Resources/QuickCapture/composer.css),
and [`editor.js`](../../Sources/Perch/Resources/QuickCapture/editor.js).
The verification mode specifically expects the staged resource at
`Contents/Resources/Perch_Perch.bundle/QuickCapture/index.html`.
...
[`editor.js`](../../Sources/Perch/Resources/QuickCapture/editor.js).
...
[`Perch.entitlements`](../../Support/Perch.entitlements) grants exactly
two capabilities:
...
`com.fantomsuj.Perch`, version values, custom URL scheme, minimum system
...
2. **“Node is always required to run Perch.”** No. The generated editor is
...
`dist/Perch.app/Contents` if present. Do not run the build script until the
audience has saved work and confirmed no `Perch` process is active. If Node
...
1. The generated Quick Capture `editor.js` is checked in and copied as a Swift
   resource.
2. `Perch` is the executable target; `PerchTests` is the test target and
   depends on `Perch`.
...
1. `Sources/Perch/Platform`
2. `Sources/Perch/Resources/QuickCapture/editor.js`
3. `Tests/PerchTests`
4. `Web/QuickCaptureEditor`
5. `Support/Perch.entitlements`
6. `docs/superpowers`
7. `node_modules`
8. `.build`
9. `dist/Perch.app`
...
|---|---|---|
| `Sources/Perch/Platform` | Authored production source | Edit for owning native adapter behavior; add focused Swift tests and run Swift verification. |
...
| `Tests/PerchTests` | Authored tests | Edit when adding/regressing native behavior; never package it into the app. |
...
| `Support/Perch.entitlements` | Configuration and security contract | Change only for an explicit capability requirement; verify signing and behavior. |
...
| `dist/Perch.app` | Staged build output | Inspect or launch for verification; do not treat it as source. |
...
- Perch is one SwiftPM executable with one Swift test target and one copied
