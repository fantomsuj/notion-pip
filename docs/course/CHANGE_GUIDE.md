...
**Invariants.** Preserve the source's case/diacritic/whitespace normalization,
including `folding(..., locale: .current)`, title matches ahead of page-ID
matches, pinned-before-recent ties, newer-before-older ties, stable page-ID
ordering, case-insensitive deduplication, active-row marking, and the stored
pinned/recent order for an empty query. Ranking is deterministic for a fixed
process locale; the current tests do not establish locale independence. Making
ranking identical across locales would be a new requirement needing an
explicit locale decision and dedicated tests.
