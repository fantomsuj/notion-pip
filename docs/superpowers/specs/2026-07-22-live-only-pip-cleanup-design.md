...
Older builds may have left derived preview files under `Application Support/NotionPiP/NativePageCache`. The app does not inspect or delete that directory at launch or during an upgrade. It removes the directory only after the user explicitly disconnects their personal Notion token.
