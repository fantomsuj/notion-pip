    ...
    private static let captureScript = #"""
        (() => {
          ...
          const token = globalThis.crypto?.randomUUID?.() ??
            `perch-${Date.now()}-${Math.random()}`;
          Object.defineProperty(editable, '__perchSelectionToken', {
            configurable: true,
            value: token,
          });
        ...
        return #"""
            (() => {
              ...
              if (!(editable instanceof Element) || !editable.isConnected ||
                  ...
                  editable.__perchSelectionToken !== snapshot.token) {
                return false;
              }
    ...
    private static func insertScript(
        ...
        return #"""
            (() => {
              ...
              if (!(editable instanceof Element) || !editable.isConnected ||
                  !editable.isContentEditable ||
                  editable.__perchSelectionToken !== snapshot.token) return false;
              const anchor = resolve(editable, snapshot.anchorPath);
              const focus = resolve(editable, snapshot.focusPath);
              ...
              try {
                ...
                const nextToken = globalThis.crypto?.randomUUID?.() ??
                  `perch-${Date.now()}-${Math.random()}`;
                Object.defineProperty(currentEditable, '__perchSelectionToken', {
                  configurable: true,
                  value: nextToken,
                });
                return {
