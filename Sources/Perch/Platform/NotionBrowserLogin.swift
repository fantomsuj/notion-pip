...
enum NotionBrowserHandoffRoute {
    ...
    // The desktop router currently ignores the authority. Perch accepts only
...
enum NotionBrowserHandoffRedemption {
    ...
    static let contentWorld = WKContentWorld.world(
        name: "com.fantomsuj.Perch.browser-handoff"
    )
    ...
    static func redeem(
        ...
        let script = """
            ...
            const cancelledAttempts =
              globalThis.__perchCancelledHandoffAttempts || new Set();
            globalThis.__perchCancelledHandoffAttempts = cancelledAttempts;
            if (cancelledAttempts.delete(attemptID)) return false;

            const controller = new AbortController();
            const controllers = globalThis.__perchHandoffControllers || new Map();
            globalThis.__perchHandoffControllers = controllers;
        ...
        return AnyCancellable { [weak webView] in
            ...
            Task { @MainActor in
                ...
                _ = try? await webView.callAsyncJavaScript(
                    """
                    const controllers = globalThis.__perchHandoffControllers;
