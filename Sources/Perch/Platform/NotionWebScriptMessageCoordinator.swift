...
enum NotionEditorActivityBridge {
    static let handlerName = "perchChromeActivity"
...
enum NotionScrollBridge {
    static let handlerName = "perchScroll"
...
enum NotionUsefulContentBridge {
    static let handlerName = "perchUsefulContent"
    static let documentIdentifierExpression =
        "window.__perchUsefulContentDocumentID ?? null"
...
final class NotionWebScriptMessageCoordinator {
    ...
    private static let editorActivityScript = #"""
        (() => {
          if (window.__perchChromeActivityInstalled) return;
    ...
    private static let scrollScript = #"""
        (() => {
          if (window.__perchScrollInstalled) return;
          window.__perchScrollInstalled = true;
          let timer = null;
          ...
          const publish = () => {
            ...
            window.webkit?.messageHandlers?.perchScroll?.postMessage({
              x: window.scrollX,
              y,
    ...
    static func usefulContentScript(timeoutMilliseconds: Int = 30_000) -> String {
        #"""
        (() => {
          if (window.__perchUsefulContentInstalled) return;
          window.__perchUsefulContentInstalled = true;

          const documentID = window.crypto?.randomUUID?.();
          if (!documentID) return;
          Object.defineProperty(window, '__perchUsefulContentDocumentID', {
            configurable: false,
            enumerable: false,
            value: documentID,
            writable: false,
