...
final class PiPRecentPagesShelfTests: XCTestCase {
    func testAccessibilityCopyDescribesCurrentAndRestorableRows() {
        XCTAssertEqual(
            ...
            "Product roadmap, Current, active Perch page"
        )
        XCTAssertEqual(
    ...
    @MainActor
    func testLoadResolvesCalendarAgainAfterTimeZoneChanges() async throws {
        var utcCalendar = Calendar(identifier: .gregorian)
        utcCalendar.timeZone = utc
        let now = try XCTUnwrap(
            utcCalendar.date(
                from: DateComponents(year: 2026, month: 8, day: 10, hour: 0, minute: 30)
            )
        )
        let active = StoredPageSnapshot(
            pageID: pageID(0),
            canonicalURL: try pageURL(0),
            displayTitle: "Active",
            timestamp: now
        )
        let previousHour = StoredPageSnapshot(
            pageID: pageID(1),
            canonicalURL: try pageURL(1),
            displayTitle: "Previous hour",
            timestamp: now.addingTimeInterval(-3_600)
        )
        let calendars = MutableRecentPagesCalendar(timeZone: utc)
        let controller = PiPRecentPagesShelfController(
            store: RecentPagesStore(
                snapshot: PiPRecentPagesSnapshot(
                    activePageID: active.pageID,
                    pages: [active, previousHour],
                    restorations: []
                )
            ),
            clock: TestDateProvider(now),
            calendarProvider: { calendars.makeCalendar() }
        )

        await controller.load()
        XCTAssertEqual(controller.items[1].recency, .yesterday)

        calendars.timeZone = try XCTUnwrap(TimeZone(secondsFromGMT: -7 * 3_600))
        await controller.load()

        XCTAssertEqual(controller.items[1].recency, .hours(1))
    }
...
@MainActor
private final class MutableRecentPagesCalendar {
