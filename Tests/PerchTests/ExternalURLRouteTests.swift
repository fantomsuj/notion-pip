...
final class ExternalURLRouteTests: XCTestCase {
    ...
    func testPercentEncodedPageURLParsesPinRoute() throws {
        let routeURL = try XCTUnwrap(
            URL(
                string: "perch://pin?url=https%3A%2F%2Fnotion.com%2FProject-\(pageID)%3Fview%3Dall%23notes&source=chrome-extension"
            )
        )
    ...
    func testMissingURLAndSourceAreRejected() throws {
        let missingURL = try XCTUnwrap(URL(string: "perch://pin?source=chrome-extension"))
        let missingSource = try XCTUnwrap(
            URL(string: "perch://pin?url=https%3A%2F%2Fwww.notion.so%2F\(pageID)")
        )
    ...
    func testInvalidNestedPageURLIsRejected() throws {
        let routeURL = try XCTUnwrap(
            URL(string: "perch://pin?url=http%3A%2F%2Fwww.notion.so%2F\(pageID)&source=chrome-extension")
        )
